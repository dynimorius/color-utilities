...
import { cie76ColorDiff, } from "../public_api";
const checkIfInExceptibleRange = (color, testColor) => {
    expect(cie76ColorDiff(color.rgb, testColor.rgb)).toBeLessThanOrEqual(PRECEPTABLE_THROUGH_CLOSE_OBESERVATION);
};
const Test = (rgb, colorName) => {
    const testColor = new Color("rgb", rgb, "all");
    test(`Checking ${colorName} from a HEX value is correct"`, () => {
        const color = new Color("hex", `#${testColor.data?.hex}`);
        checkIfInExceptibleRange(color, testColor);
    });
    test(`Checking ${colorName} from a HLS value is correct"`, () => {
        const color = new Color("hsl", testColor.data?.hsl);
        checkIfInExceptibleRange(color, testColor);
    });
    test(`Checking ${colorName} from a HSV value is correct"`, () => {
        const color = new Color("hsv", testColor.data?.hsv);
        checkIfInExceptibleRange(color, testColor);
    });
    test(`Checking ${colorName} from a HWB value is correct"`, () => {
        const color = new Color("hwb", testColor.data?.hwb);
        checkIfInExceptibleRange(color, testColor);
    });
    test(`Checking ${colorName} from a LUV value is correct"`, () => {
        const color = new Color("luv", testColor.data?.luv);
        checkIfInExceptibleRange(color, testColor);
    });
    test(`Checking ${colorName} from a LAB value is correct"`, () => {
        const color = new Color("lab", testColor.data?.lab);
        checkIfInExceptibleRange(color, testColor);
    });
    test(`Checking ${colorName} from a LCH(ab) value is correct"`, () => {
        const color = new Color("lch_ab", testColor.data?.lch_ab);
        checkIfInExceptibleRange(color, testColor);
    });
    test(`Checking ${colorName} from a LCH(uv) value is correct"`, () => {
        const color = new Color("lch_uv", testColor.data?.lch_uv);
        checkIfInExceptibleRange(color, testColor);
    });
    test(`Checking ${colorName} from a RYB value is correct"`, () => {
        const color = new Color("ryb", testColor.data?.ryb);
        checkIfInExceptibleRange(color, testColor);
    });
    test(`Checking ${colorName} from a XYZ value is correct"`, () => {
        const color = new Color("xyz", testColor.data?.xyz);
        checkIfInExceptibleRange(color, testColor);
    });
    test(`Checking ${colorName} from a XYY value is correct"`, () => {
        const color = new Color("xyy", testColor.data?.xyy);
        checkIfInExceptibleRange(color, testColor);
    });
    test(`Checking ${colorName} from a CMY value is correct"`, () => {
        const color = new Color("cmy", testColor.data?.cmy);
        checkIfInExceptibleRange(color, testColor);
    });
    test(`Checking ${colorName} from a CMYK value is correct"`, () => {
        const color = new Color("cmyk", testColor.data?.cmyk);
        checkIfInExceptibleRange(color, testColor);
    });
    test(`Checking ${colorName} from a HCY value is correct"`, () => {
        const color = new Color("hcy", testColor.data?.hcy);
        checkIfInExceptibleRange(color, testColor);
    });
    test(`Checking ${colorName} from a Adobe 98 RGB value is correct"`, () => {
        const color = new Color("adobe_98_rgb", testColor.data?.adobe_98_rgb);
        checkIfInExceptibleRange(color, testColor);
    });
    test(`Checking ${colorName} from a Apple RGB value is correct"`, () => {
        const color = new Color("apple_rgb", testColor.data?.apple_rgb);
        checkIfInExceptibleRange(color, testColor);
    });
    test(`Checking ${colorName} from a Best RGB value is correct"`, () => {
        const color = new Color("best_rgb", testColor.data?.best_rgb);
        checkIfInExceptibleRange(color, testColor);
    });
    test(`Checking ${colorName} from a Beta RGB value is correct"`, () => {
        const color = new Color("beta_rgb", testColor.data?.beta_rgb);
        checkIfInExceptibleRange(color, testColor);
    });
    test(`Checking ${colorName} from a Bruce RGB value is correct"`, () => {
        const color = new Color("bruce_rgb", testColor.data?.bruce_rgb);
        checkIfInExceptibleRange(color, testColor);
    });
    test(`Checking ${colorName} from a CIE RGB value is correct"`, () => {
        const color = new Color("cie_rgb", testColor.data?.cie_rgb);
        checkIfInExceptibleRange(color, testColor);
    });
    test(`Checking ${colorName} from a Color Match RGB value is correct"`, () => {
        const color = new Color("color_match_rgb", testColor.data?.color_match_rgb);
        checkIfInExceptibleRange(color, testColor);
    });
    test(`Checking ${colorName} from a Done RGB 4 value is correct"`, () => {
        const color = new Color("don_rgb_4", testColor.data?.don_rgb_4);
        checkIfInExceptibleRange(color, testColor);
    });
    test(`Checking ${colorName} from a ECI RGB v2 value is correct"`, () => {
        const color = new Color("eci_rgb_v2", testColor.data?.eci_rgb_v2);
        checkIfInExceptibleRange(color, testColor);
    });
    test(`Checking ${colorName} from a Etka Space ps5 value is correct"`, () => {
        const color = new Color("etka_space_ps5", testColor.data?.etka_space_ps5);
        checkIfInExceptibleRange(color, testColor);
    });
    test(`Checking ${colorName} from a NTSC RGB value is correct"`, () => {
        const color = new Color("ntsc_rgb", testColor.data?.ntsc_rgb);
        checkIfInExceptibleRange(color, testColor);
    });
    test(`Checking ${colorName} from a Pal/Secam RGB value is correct"`, () => {
        const color = new Color("pal_secam_rgb", testColor.data?.pal_secam_rgb);
        checkIfInExceptibleRange(color, testColor);
    });
    test(`Checking ${colorName} from a Pro Photo RGB value is correct"`, () => {
        const color = new Color("pro_photo_rgb", testColor.data?.pro_photo_rgb);
        checkIfInExceptibleRange(color, testColor);
    });
    test(`Checking ${colorName} from a SMPTE/C RGB value is correct"`, () => {
        const color = new Color("smpte_c_rgb", testColor.data?.smpte_c_rgb);
        checkIfInExceptibleRange(color, testColor);
    });
    test(`Checking ${colorName} from a Wide Gamut RGB value is correct"`, () => {
        const color = new Color("wide_gamut_rgb", testColor.data?.wide_gamut_rgb);
        checkIfInExceptibleRange(color, testColor);
    });
    test(`Checking ${colorName} from a YCbCr BT601 value is correct"`, () => {
        const color = new Color("ycbcr_BT601", testColor.data?.ycbcr_BT601);
        checkIfInExceptibleRange(color, testColor);
    });
    test(`Checking ${colorName} from a YcCbcCrc value is correct"`, () => {
        const color = new Color("yccbccrc", testColor.data?.yccbccrc);
        checkIfInExceptibleRange(color, testColor);
    });
    test(`Checking ${colorName} from a YCgCo value is correct"`, () => {
        const color = new Color("ycocg", testColor.data?.ycocg);
        checkIfInExceptibleRange(color, testColor);
    });
    test(`Checking ${colorName} from a YCDbDr value is correct"`, () => {
        const color = new Color("ydbdr", testColor.data?.ydbdr);
        checkIfInExceptibleRange(color, testColor);
    });
    test(`Checking ${colorName} from a YIQ value is correct"`, () => {
        const color = new Color("yiq", testColor.data?.yiq);
        checkIfInExceptibleRange(color, testColor);
    });
    test(`Checking ${colorName} from a YPbPr value is correct"`, () => {
        const color = new Color("ypbpr", testColor.data?.ypbpr);
        checkIfInExceptibleRange(color, testColor);
    });
    test(`Checking ${colorName} from a TSL value is correct"`, () => {
        const color = new Color("tsl", testColor.data?.tsl);
        checkIfInExceptibleRange(color, testColor);
    });
    test(`Checking ${colorName} from a UVW value is correct"`, () => {
        const color = new Color("uvw", testColor.data?.uvw);
        checkIfInExceptibleRange(color, testColor);
    });
    test(`Checking ${colorName} from a Hunter's Lab value is correct"`, () => {
        const color = new Color("hunter_lab", testColor.data?.hunter_lab);
        checkIfInExceptibleRange(color, testColor);
    });
    test(`Checking ${colorName} from a LMS value is correct"`, () => {
        const color = new Color("lms", testColor.data?.lms);
        checkIfInExceptibleRange(color, testColor);
    });
};
const ErrorTest = (rgb, colorName) => {
