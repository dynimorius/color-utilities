...
import { cie76ColorDiff, sRgbToYDbDr, yDbDrToSrgb } from "../public_api";
const Test = (rgb, colorName) => {
    test(`Checking RGB <-> YDbDr conversions for ${colorName}`, () => {
        expect(cie76ColorDiff(yDbDrToSrgb(sRgbToYDbDr(rgb)), rgb)).toBeLessThanOrEqual(NOT_PERCEPTIBLE_BY_HUMAN_EYE);
    });
};
