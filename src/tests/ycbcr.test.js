...
import { cie76ColorDiff, sRgbToYCbCrBT601, sRgbToYCbCrBT709, yCbCrBT601ToSrgb, yCbCrBT709ToSrgb, } from "../public_api";
const TestBT601 = (rgb, colorName) => {
    test(`Checking RGB <-> YCbCr BT601 conversions of for ${colorName}`, () => {
        expect(cie76ColorDiff(yCbCrBT601ToSrgb(sRgbToYCbCrBT601(rgb)), rgb)).toBeLessThanOrEqual(NOT_PERCEPTIBLE_BY_HUMAN_EYE);
    });
};
...
const TestBT709 = (rgb, colorName) => {
    test(`Checking RGB <-> YCbCr BT709 conversions for ${colorName}`, () => {
        expect(cie76ColorDiff(yCbCrBT709ToSrgb(sRgbToYCbCrBT709(rgb)), rgb)).toBeLessThanOrEqual(NOT_PERCEPTIBLE_BY_HUMAN_EYE);
    });
};
