...
const Test3 = (xyz: XYZ, colorName: string) => {
  test(`Checking LCH(ab) <-> LAB conversions for ${colorName}`, () => {
    const lch = xyzToLch_uv(xyz);
    expect(
      deltaECIE76Lch(labToLch_ab(lch_abToLab(lch)), lch)
    ).toBeLessThanOrEqual(NOT_PERCEPTIBLE_BY_HUMAN_EYE);
  });
};
