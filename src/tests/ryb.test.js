...
import { cie76ColorDiff, rybToSrgb, sRgbToRyb } from "../public_api";
const Test = (rgb, colorName) => {
    test(`Checking RGB <-> RYB conversions for ${colorName}`, () => {
        expect(cie76ColorDiff(rybToSrgb(sRgbToRyb(rgb)), rgb)).toBeLessThanOrEqual(NOT_PERCEPTIBLE_BY_HUMAN_EYE);
    });
};
