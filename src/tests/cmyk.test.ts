import { cmykToRgb, comparativeDistance, sRgbToCmyk } from "../public_api";

const Test = (rgb: { red: number; green: number; blue: number }) => {
  test(`Checking RGB <-> CMYK conversions of R: ${rgb.red} G: ${rgb.green} B: ${rgb.blue}`, () => {
    expect(
      comparativeDistance(cmykToRgb(sRgbToCmyk(rgb)), rgb)
