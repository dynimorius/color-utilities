 ...
 */
export const yCbCrBT601ToSrgb = ({ Y, Cb, Cr }) => {
    const { red, green, blue } = matrixByVectorObjMultiAsSpace(CB_CR_CONVERSION_MATRICES.BT_601_YCBCR_TO_RGB, { Y: Y - 16, Cb: Cb - 128, Cr: Cr - 128 }, ["red", "green", "blue"]);
    return {
        red: Math.round(red),
 ...
 */
export const yCbCrBT601ToXvYcc = ({ Y, Cb, Cr }) => {
    Y = Y > 1 ? Y / 255 : Y;
    Cb = Cb > 1 ? Cb / 255 : Cb;
    Cr = Cr > 1 ? Cr / 255 : Cr;
    return {
...
export const yCbCrBT601ToYPbPr = ({ Y, Cb, Cr }) => {
    Y = Y > 1 ? Y / 255 : Y;
    Cb = Cb > 1 ? Cb / 255 : Cb;
    Cr = Cr > 1 ? Cr / 255 : Cr;
    return {
 ...
 * @returns {RGB}                   - sRGB values for a color
 */
export const yCbCrBT709ToSrgb = ({ Y, Cb, Cr }) => {
    const { red, green, blue } = matrixByVectorObjMultiAsSpace(CB_CR_CONVERSION_MATRICES.BT_709_YCBCR_TO_RGB, { Y, Cb, Cr }, ["red", "green", "blue"]);
    return {
        red: Math.round(red),
        green: Math.round(green),
        blue: Math.round(blue),
