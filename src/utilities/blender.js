 ...
 */
export const blend = (rgb1, rgb2, weight = 0.5) => {
    return {
...
export class Blender {
    constructor(color1, color2, oprions) {
        ...
        color1 = checkAndFormat(type1, color1);
        const type2 = getColorType(color2);
        color2 = checkAndFormat(type2, color2);
        this.rgb1 =
            type1 === "rgb"
                ? color1
                : toRgbConverters[getColorType(color1)](color1);
        this.rgb2 =
            type2 === "rgb"
                ? color2
                : toRgbConverters[getColorType(color2)](color2);
        const weight = oprions.weight ? oprions.weight : 0.5;
        this.color =
            oprions.returnType === "rgb" || !oprions.returnType
                ? blend(this.rgb1, this.rgb2, weight)
                : (colorConverters[oprions.returnType]
                    ?.fun)(blend(this.rgb1, this.rgb2, weight));
        this.blendData = {
            color1: {
                data: color1,
                rgb: checkAndFormat("rgb", this.rgb1),
                amount: weight,
            },
            color2: {
                data: color2,
                rgb: checkAndFormat("rgb", this.rgb2),
                amount: 1 - weight,
            },
