 ...
 *                             HEX
 * *****************************************************************/
const sRgbToHex = ({ red, green, blue }) => {
...
const sRgbaToHex = ({ red, green, blue, alpha }) => {
    const integer = ((Math.round(red) & 0xff) << 16) +
        ((Math.round(green) & 0xff) << 8) +
        (Math.round(blue) & 0xff);
    const string = integer.toString(16).toUpperCase();
