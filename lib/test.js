...
const printData = (colors) => {
    colors.forEach((color) => {
        if (!checkRgb(color.rgb)) {
            ...
            console.log(`\n${name}\n`, `----------------------------------------------\n`, color.data().rgb, `\n`, color.data().xyz, `\n`, color.data().hex, `\n`, `\n----------------------------------------------\n`, `Diff\n`, `{ red: ${color.rgb.red - testColor.red}, green: ${color.rgb.green - testColor.green}, blue: ${color.rgb.blue - testColor.blue}} \n`, `{ x: ${color.xyz.x - checkColor.data().xyz.x}, y: ${color.xyz.y - checkColor.data().xyz.y}, z: ${color.xyz.z - checkColor.data().xyz.z}} \n ${checkColor.data().hex} - ${color.hex} 
        `, `\n----------------------------------------------\n`);
        }
        else {
            const name = Object.keys(color)[0].replace("_", " ").toUpperCase();
