...
*/
const deltaECIE94Lab = (lab1, lab2, weights) => {
    ...
    const cab = c1 - c2;
    const hab = Math.sqrt(Math.pow(deltaA, 2) +
        Math.pow(deltaB, 2) -
        Math.pow(cab, 2)) ?? 0;
    const L = (lab1.luminance - lab2.luminance) / lightness;
    const a = cab / (chroma * sc);
    const b = hab / sh;
    return Math.sqrt(Math.pow(L, 2) +
        Math.pow(a, 2) +
        Math.pow(b, 2));
};
exports.deltaECIE94Lab = deltaECIE94Lab;
