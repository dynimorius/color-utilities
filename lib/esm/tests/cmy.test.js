...
import { cie76ColorDiff, cmyToSRgb, sRgbToCmy } from "../public_api";
const Test = (rgb, colorName) => {
    test(`Checking RGB <-> CMY conversions for ${colorName}`, () => {
        expect(cie76ColorDiff(cmyToSRgb(sRgbToCmy(rgb)), rgb)).toBeLessThanOrEqual(NOT_PERCEPTIBLE_BY_HUMAN_EYE);
    });
};
