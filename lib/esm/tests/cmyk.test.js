...
import { cie76ColorDiff, cmykToRgb, sRgbToCmyk } from "../public_api";
const Test = (rgb, colorName) => {
    test(`Checking RGB <-> CMYK conversions for ${colorName}`, () => {
        expect(cie76ColorDiff(cmykToRgb(sRgbToCmyk(rgb)), rgb)).toBeLessThanOrEqual(NOT_PERCEPTIBLE_BY_HUMAN_EYE);
    });
};
